...
func TestClaudeConfigurationOnlyOffersAdvertisedModels(t *testing.T) {
...
`)
	...
	// newest model without a local catalog.
	...
	if len(models) != 4 {
		t.Fatalf("unexpected models not advertised by the CLI: %+v", models)
	}
...
func TestClaudeConfigurationDoesNotInventModelsWithoutCLIOptions(t *testing.T) {
	if models := parseClaudeModelOptions("Options:\n  --help  Show help"); len(models) != 0 {
