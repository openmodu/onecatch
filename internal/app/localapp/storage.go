...
func (a *App) RevealDataRoot() error {
...
func (a *App) PreviewCleanup(ctx context.Context, input CleanupPreviewInput) (CleanupPreview, error) {
...
func (a *App) ExecuteCleanup(ctx context.Context, token string) (CleanupResult, error) {
	a.cleanupMu.Lock()
...
func (a *App) resetCleanupPlan(token string) {
	a.cleanupMu.Lock()
	plan, ok := a.cleanupPlans[token]
	if ok {
		plan.Running = false
...
func (a *App) ExportDiagnostics(ctx context.Context, input DiagnosticsExportInput) (DiagnosticsExport, error) {
	settings, err := a.settings.Get(ctx)
	if err != nil {
