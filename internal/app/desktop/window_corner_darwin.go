...
static void oneshotInstallTitlebarDoubleClick(NSWindow *window) {
	...
	// matched. Needing two clicks to match means a lone click is held for the
	...
	// click. gestureRecognizerShouldBegin only limits where the gesture may
	// begin (the titlebar strip); it does not stop the delay from applying to the
	// whole content view. Setting NO lets the recognizer observe the titlebar
	// double-click without ever intercepting normal clicks.
