...
本仓库采用”需求 -> 实现 + 文档同步 -> 提交”的交付方式。产品设计、技术设计、编码、测试和交付记录都应沉淀在 repo 内，避免只存在于聊天上下文。issue 和技术方案文档随代码一同产出或更新，不要求文档先于代码。

**不要在回复中输出图片、图表或架构图，浪费 token。描述用文字即可。**
...
技术方案放在 `design/docs/` 目录，可以集中维护在 `design/docs/TECHNICAL_DESIGNS.md`，也可以在复杂阶段拆成独立文件。

规则：

- 每个进入开发的 issue 应能链接到一份技术方案。
- 技术方案至少写清关键改动点（后端、前端、数据模型、API/binding）、错误码和验证计划。
...
4. issue 必须有清晰验收标准。issue 和技术方案文档随代码一同创建或更新，不强制先写文档再编码。
...
- 实现前或实现过程中确认对应 PRD 和 issue。
- issue 的”技术设计”在实现完成后更新以反映实际情况，不必提前写完整才动代码。
- 设计变更在工作过程中同步更新 issue 的”技术设计”。
- 范围变化同步更新 issue 的”范围”或”非目标”。
- 实现完成后更新”交付记录”。
...
- 原型或 mock 中的行为不能直接视为生产规则，生产规则必须写入 PRD 或 issue。
