...
- 每个进入开发的 issue 必须能链接到一份技术方案。
- 技术方案必须先于代码实现提交或至少先于实现落盘完成。
- 技术方案应包含后端 API、Wails binding、客户端 SDK、数据模型、前后端联调点、错误码和验证计划。
- 涉及登录、支付、扣次、订单、交付物、持久化和 worker 的 issue，必须明确接口入参、响应、错误和幂等/鉴权边界。
- 未完成技术方案前，不应开始编码；如果是紧急修复，也必须在同一提交内补齐技术方案。
...
- 开始前阅读对应 PRD 和 issue。
- 开始编码前先新增或更新技术方案，并在 issue 的“来源”里链接技术方案。
- 技术方案至少写清 API、binding、数据模型、错误码、前后端联调点和测试计划。
