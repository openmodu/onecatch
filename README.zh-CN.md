...
电脑端在「设置 › 手机连接」开启服务并生成配对码，iOS 或 Android 首次输入电脑地址和配对码。配对后，电脑 IP 变化时，手机会通过局域网发现查找新地址，校验原来的证书和 Worker 身份，再保存新地址，不需要重复配对。已有 HTTPS 配对记录可继续使用；电脑和手机都需要升级到包含此功能的版本。

- 手机与电脑需处于可互相访问的局域网。iOS 和 macOS 提示本地网络权限时请选择允许；Android 只在应用前台开启 Wi-Fi 多播接收。
- 路由器的客户端隔离、访客 Wi-Fi、VPN 或防火墙可能阻止发现。允许局域网 mDNS（UDP 5353）和电脑连接端口；无法调整网络时，仍可输入新地址重新配对。
- 自动发现只提供候选地址，不建立新的信任。证书变化时不会自动接受，需要确认电脑身份后重新配对。
- 断线不会自动重发启动任务、删除等写操作。进行中的连接仍可能中断，恢复后需重新读取任务状态。
- 独立 `onecatch worker` 仅在启用 TLS 且监听通配地址（如 `--listen :9231`）时广播；绑定具体旧 IP 的进程无法随 DHCP 变化继续监听。明文 HTTP 配对不支持自动迁移。
