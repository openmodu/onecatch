...
for tool in codesign hdiutil lipo plutil shasum; do
...
for input in "$BINARY" "$WORKER_BINARY" "$SHELL_BINARY" "$ASKPASS_BINARY" "$INFO_PLIST" "$ASSETS_CAR" "$ICON_FILE" "$VERSION_FILE"; do
    if [ ! -f "$input" ]; then
        echo "error: required build input not found: $input" >&2
        exit 1
    fi
done
for executable in "$BINARY" "$WORKER_BINARY" "$SHELL_BINARY" "$ASKPASS_BINARY"; do
