...
export function CommitAndPush(input) {
    return $Call.ByID(3967383100, input).then(/** @type {($result: any) => any} */(($result) => {
...
export function Diff(workspaceID, staged) {
    return $Call.ByID(4071402393, workspaceID, staged);
}
...
export function StageAll(workspaceID) {
    return $Call.ByID(3749334327, workspaceID);
}
...
export function Status(workspaceID) {
    return $Call.ByID(1242223788, workspaceID).then(/** @type {($result: any) => any} */(($result) => {
        return $$createType1($result);
    }));
}
