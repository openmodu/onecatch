...
function App() {
  ...
  return <div className="app-frame">
    ...
    {workerModal && <Modal title={t("worker.modalTitle")} subtitle={t("worker.modalSubtitle")} onClose={() => setWorkerModal(false)}><div className="form-stack"><label>{t("worker.id")}<input value={workerForm.id} onChange={(event) => setWorkerForm((form) => ({ ...form, id: event.target.value }))} placeholder="mac-mini" /></label><label>{t("worker.name")}<input value={workerForm.name} onChange={(event) => setWorkerForm((form) => ({ ...form, name: event.target.value }))} placeholder="Build Mac mini" /></label><label>{t("worker.baseUrl")}<input value={workerForm.baseUrl} onChange={(event) => setWorkerForm((form) => ({ ...form, baseUrl: event.target.value }))} placeholder="https://192.168.1.20:9231" /></label><label>{t("worker.bearerToken")}<input type="password" value={workerForm.token} onChange={(event) => setWorkerForm((form) => ({ ...form, token: event.target.value }))} placeholder={t("worker.keepToken")} /></label><label>{t("worker.serverFingerprint")}<input value={workerForm.serverCertificateSha256} onChange={(event) => setWorkerForm((form) => ({ ...form, serverCertificateSha256: event.target.value }))} placeholder={t("worker.serverFingerprintPlaceholder")} /></label><label>{t("worker.caFile")}<input value={workerForm.caFile} onChange={(event) => setWorkerForm((form) => ({ ...form, caFile: event.target.value }))} placeholder="/path/to/ca.pem" /></label><label>{t("worker.serverName")}<input value={workerForm.serverName} onChange={(event) => setWorkerForm((form) => ({ ...form, serverName: event.target.value }))} placeholder="worker.example.internal" /></label><div className="two-fields"><label>{t("worker.clientCertFile")}<input value={workerForm.clientCertFile} onChange={(event) => setWorkerForm((form) => ({ ...form, clientCertFile: event.target.value }))} placeholder="/path/to/client.pem" /></label><label>{t("worker.clientKeyFile")}<input type="password" value={workerForm.clientKeyFile} onChange={(event) => setWorkerForm((form) => ({ ...form, clientKeyFile: event.target.value }))} placeholder="/path/to/client-key.pem" /></label></div><label className="checkbox-label"><input type="checkbox" checked={workerForm.enabled} onChange={(event) => setWorkerForm((form) => ({ ...form, enabled: event.target.checked }))} />{t("worker.enableScheduling")}</label><div className="modal-actions"><Action tone="muted" onClick={() => setWorkerModal(false)}>{t("common.cancel")}</Action><Action tone="primary" disabled={busy === "worker"} onClick={saveWorker}>{t("worker.save")}</Action></div></div></Modal>}
