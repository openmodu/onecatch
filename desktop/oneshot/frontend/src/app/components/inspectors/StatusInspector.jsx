...
export default function StatusInspector({ detail, queuedTask, queuePosition, notify }) {
  ...
  return <div className="status-inspector">
    <div className="status-hero"><StatusPill status={run.status} active={detail.active} /><strong>{currentStep?.name || run.currentStepId || "—"}</strong><small>{currentStep?.runtime || "agent"}</small></div>
    <div className="run-metric-grid">
      ...
      <div><span>{t("inspector.duration")}</span><strong>{formatDuration(duration)}</strong></div>
      <div><span>{t("inspector.rounds")}</span><strong>{stepRuns.length}</strong></div>
    </div>
    <section className="inspector-block">
      <Kicker>{t("inspector.workflow")}</Kicker>
      {(workflow.steps || []).map((step, index) => {
        ...
        return <div className={`inspector-flow-row ${step.id === run.currentStepId ? "current" : ""}`} key={step.id}><b>{String(index + 1).padStart(2, "0")}</b><span><strong>{step.name}</strong><small>{step.runtime} · {t(statusKey(stepStatus), { defaultValue: stepStatus })}</small></span></div>;
      })}
    </section>
    <section className="inspector-block">
      <Kicker>{t("inspector.resumeSessions")}</Kicker>
