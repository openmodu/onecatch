...
export default function Composer({
  ...
  return <div className="workbench-composer" onMouseDown={preserveComposerFocus}>
    ...
    {attachments.length > 0 && <div className="composer-attachments">{attachments.map((path) => <span className="attachment-chip" key={path} title={path}>{fileName(path)}<Action size="compact" tone="danger" onClick={() => onRemoveAttachment(path)}>{t("common.remove")}</Action></span>)}</div>}
