...
export default function Composer({
  ...
  return <div className="workbench-composer" onMouseDown={preserveComposerFocus}>
    ...
    <div className={`workbench-composer-input ${editable ? "" : "disabled"}`.trim()}><span aria-hidden="true">&gt;</span><textarea aria-label="补充运行指令" value={draft} disabled={!editable} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter" && (canSend || runStatus === "paused")) send("queue"); }} placeholder={runStatus === "running" ? "补充下一轮指令，或打断当前轮次优先插入…" : runStatus === "paused" ? "补充指令并恢复运行…" : "这次运行已经结束"} /></div>
