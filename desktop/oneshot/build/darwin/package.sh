...
BUILD_ID=${BUILD_ID:-$(git -C "$PROJECT_DIR" rev-parse --short HEAD 2>/dev/null || printf 'dev')}
ARCHITECTURES=$(lipo -archs "$BINARY")
ARCH_LABEL=$(printf '%s' "$ARCHITECTURES" | tr ' ' '-')
OUTPUT_ZIP=${OUTPUT_ZIP:-$PROJECT_DIR/bin/$APP_NAME-$VERSION-$BUILD_ID-$ARCH_LABEL.zip}
