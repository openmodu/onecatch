...
import { Activity, FileCode2, GitBranch, ListTree } from "lucide-react";
...
export default function InspectorPanel({ className = "", mode, workspaceID, detail, queuedTask, queuePosition = 0, draft = false, runWorkerID = "", notify, onOpenTerminal, onDirtyChange, actions = null }) {
  ...
  return <aside className={className} aria-label={t("inspector.aria")}>
    ...
    <div className={`workbench-inspector-body min-h-0 ${tab === "files" ? "overflow-hidden" : "overflow-y-auto"}`} id="workbench-inspector-content">
      ...
      {tab === "status" ? <StatusInspector detail={draft ? null : detail} queuedTask={draft ? null : queuedTask} queuePosition={draft ? 0 : queuePosition} draft={draft} notify={notify} onOpenTerminal={onOpenTerminal} />
        : tab === "git" ? <GitInspector mode={mode} workspaceID={workspaceID} runWorkerID={draft ? "" : runWorkerID} notify={notify} />
