...
export default function Composer({
  ...
  return <div className="workbench-composer shrink-0 border-t bg-card px-4 py-3" onMouseDown={preserveComposerFocus}>
    ...
    <div className={`workbench-composer-input flex rounded-md border bg-background px-3 py-2 transition-colors focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/50 ${editable ? "" : "disabled opacity-60"}`.trim()}><textarea className="min-h-14 w-full resize-none border-0 bg-transparent p-0 text-sm leading-relaxed outline-none focus:outline-none" aria-label={t("composer.aria")} value={draft} disabled={!editable} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter" && (canSend || runStatus === "paused")) send("queue"); }} placeholder={runStatus === "running" ? t("composer.runningPlaceholder") : runStatus === "paused" ? t("composer.pausedPlaceholder") : t("composer.finishedPlaceholder")} /></div>
