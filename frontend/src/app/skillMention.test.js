...
import {
  ...
  remarkSkillMentions,
  splitSkillMentions,
} from "./skillMention.js";

test("opens a skill trigger at the beginning or after whitespace", () => {
  assert.deepEqual(findSkillTrigger("$git"), { start: 0, caret: 4, query: "git" });
  assert.deepEqual(findSkillTrigger("请使用 $git"), { start: 4, caret: 8, query: "git" });
  assert.deepEqual(findSkillTrigger("第一行\n$doc"), { start: 4, caret: 8, query: "doc" });
});

test("does not open a skill trigger inside ordinary text", () => {
  assert.equal(findSkillTrigger("price$usd"), null);
  assert.equal(findSkillTrigger("$git commit"), null);
});
...
test("filters by invocation name before descriptions", () => {
  ...
  assert.deepEqual(filterSkills(skills, "git").map((item) => item.name), ["git-commit", "other"]);
});

test("replaces the active trigger with a skill marker and trailing space", () => {
  const value = "请使用 $git 完成";
  const trigger = findSkillTrigger(value, 8);
  assert.deepEqual(insertSkill(value, trigger, "git-commit"), {
...
test("splits valid skill mentions without treating currency or embedded dollars as skills", () => {
  assert.deepEqual(splitSkillMentions("$git-commit 完成，价格 price$usd，另用 $a.stock:v2"), [
