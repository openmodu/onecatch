...
function runtimeLabel(runtime) {
  return RUNTIMES.find((item) => item.id === runtime)?.label || runtime || "运行时";
}
...
// The pull's own header: it lives above the list and is only as tall as the
// finger has dragged.
function PullIndicator({ refreshing }) {
  return <div className="mobile-pull-indicator" aria-hidden={!refreshing}>
    {refreshing ? <><LoaderCircle className="animate-spin" />正在刷新…</> : <ArrowDown />}
  </div>;
}
...
function WorkerSwitchSheet({ open, workers, selectedWorkerID, healthByID, onSelect, onPair, onClose }) {
  ...
  return <div className="mobile-sheet-backdrop" role="presentation" onPointerDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="mobile-sheet mobile-worker-switch-sheet" role="dialog" aria-modal="true" aria-labelledby="worker-switch-title">
      <div className="mobile-sheet-handle" />
      <header><div><small>已配对的电脑</small><h2 id="worker-switch-title">切换机器</h2></div><button type="button" className="mobile-icon-button" aria-label="关闭" onClick={onClose}><X /></button></header>
      <div className="mobile-worker-switch-list">
        {workers.map((worker) => {
          const health = healthByID[worker.id];
          return <button type="button" className={`mobile-worker-switch-row ${worker.id === selectedWorkerID ? "selected" : ""}`} key={worker.id} onClick={() => { onSelect(worker.id); onClose(); }}>
            <StatusDot online={Boolean(health)} />
            <span><strong>{worker.name || workerLabel(worker)}</strong><small>{health ? `在线 · ${health.latencyMilliseconds}ms` : "离线"}</small></span>
            {worker.id === selectedWorkerID && <Check />}
          </button>;
        })}
      </div>
...
function SessionList({ workspace, conversations, query, onOpen, onActions, onNew }) {
  const normalized = query.trim().toLowerCase();
  ...
  return <div className="mobile-page mobile-session-page">
    <div className="mobile-session-list">
      {visible.map((conversation) => <SessionRow key={conversation.id} conversation={conversation} onOpen={onOpen} onActions={onActions} />)}
    </div>
...
function AgentEvent({ run, event, index, permissionBusy, onRespond }) {
  ...
  return <details className={`mobile-event-detail ${summary.failed ? "error" : ""}`}>
    <summary><ChevronRight />{line}</summary>
    {summary.body && <pre>{summary.body}</pre>}
    {summary.result && <pre className="mobile-event-result">{summary.result}</pre>}
  </details>;
}
...
function ConversationView({ conversation, workspace, snapshot, sharedRuns, prompt, setPrompt, pending, queuing, busy, permissionBusy, runtime, transcriptNotice, onOpenContext, onStart, onQueue, onDequeue, onInterrupt, onRespond, onLoadEarlier }) {
  ...
  const queueable = Boolean((running || pending) && sharedRuns);
  const queued = [...(running?.queued || []), ...queuing];
  const action = mobileComposerAction({ prompt, running, pending, sharedRuns, busy,
      ...
      <div className="mobile-composer">
        ...
        <div className="mobile-composer-footer"><button type="button" className="mobile-context-button" onClick={onOpenContext}><Plus /><span>{workspaceLabel(workspace)}</span><b><RuntimeHarnessIcon harness={runtime} size={13} />{runtime}</b></button>
          <div className="mobile-composer-actions">
            <button type="button" className={`mobile-send-button${action.mode === "running" ? " running" : ""}`}
              aria-label={action.mode === "running" ? (action.interruptible ? "运行中，点击停止" : "运行中") : action.mode === "queue" ? "排队发送" : "发送"}
              aria-disabled={action.mode === "running" && !action.interruptible || undefined}
...
// Projects fold, the way they do in the desktop sidebar: a drawer that lists
// every session of every project is a wall of text on a phone. The open
// project starts expanded, and what the reader folds stays folded.
function Sidebar({ open, workspaces, conversations, selectedConversationID, workspaceID, health, onClose, onHome, onUsage, onWorkspace, onConversation, onNew, onWorkers }) {
  const [expanded, setExpanded] = useState(() => new Set(workspaceID ? [workspaceID] : []));
  useEffect(() => {
    if (!open || !workspaceID) return;
    setExpanded((current) => current.has(workspaceID) ? current : new Set(current).add(workspaceID));
  }, [open, workspaceID]);
      ...
      <div className="mobile-drawer-scroll">
        {workspaces.map((workspace) => {
          ...
          return <section className="mobile-drawer-group" key={workspace.id}>
            <button type="button" className={`mobile-drawer-workspace ${isOpen ? "open" : ""}`} aria-expanded={isOpen} onClick={() => toggle(workspace.id)}><ChevronRight className="mobile-drawer-caret" /><strong>{workspaceLabel(workspace)}</strong><RemoteWorkspaceBadge workspace={workspace} /><span>{sessions.length}</span></button>
            {isOpen && sessions.slice(0, 8).map((conversation) => <button type="button" className={conversation.id === selectedConversationID ? "selected" : ""} key={conversation.id} onClick={() => { onConversation(conversation.id); onClose(); }}><span className="mobile-drawer-session-title">{conversation.title}</span>{conversation.status === "running" && <StatusDot online running />}</button>)}
            {isOpen && sessions.length > 8 && <button type="button" className="mobile-drawer-more" onClick={() => { onWorkspace(workspace.id); onClose(); }}>查看全部 {sessions.length} 个会话<ChevronRight /></button>}
          </section>;
        })}
      </div>
...
function WorkersSheet({ open, workers, selectedWorkerID, healthByID, busy, onClose, onPair, onRefresh, onDelete, onSelect }) {
  if (!open) return null;
  return <div className="mobile-sheet-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}>
...
// A list this quiet has nowhere to put a row's own actions, so a long press
// opens them.
function SessionActionSheet({ conversation, onRename, onDelete, onClose }) {
  if (!conversation) return null;
  return <div className="mobile-popover-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="mobile-more-menu">
      <div className="mobile-menu-status"><small>会话</small><strong>{conversation.title}</strong></div>
      <hr />
      <button type="button" onClick={() => { onRename(conversation); onClose(); }}><span /><Pencil />重命名</button>
      <button type="button" className="destructive" onClick={() => { onDelete(conversation); onClose(); }}><span /><Trash2 />删除会话</button>
    </section>
  </div>;
}

function RenameSheet({ conversation, busy, onSubmit, onClose }) {
  ...
  return <div className="mobile-sheet-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="mobile-sheet">
      <div className="mobile-sheet-handle" />
      <header><div><small>会话</small><h2>重命名</h2></div><button type="button" className="mobile-icon-button" aria-label="关闭" onClick={onClose}><X /></button></header>
      <label><span>标题</span><Input value={title} maxLength={160} onChange={(event) => setTitle(event.target.value)} /></label>
      <Button className="mobile-main-action" disabled={!trimmed || busy} onClick={() => onSubmit(conversation, trimmed)}>保存</Button>
    </section>
  </div>;
}
...
function ConversationMenu({ open, conversation, workspace, health, snapshot, runtime, model, onNew, onSettings, onRename, onDelete, onClose }) {
  if (!open) return null;
  ...
  return <div className="mobile-popover-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="mobile-more-menu">
      ...
      <div className="mobile-menu-status">
        ...
        {usage.total > 0 && <span>输入 {compactTokens(usage.input)} · 输出 {compactTokens(usage.output)}</span>}
        {usage.cached > 0 && <span>缓存命中 {compactTokens(usage.cached)} · 命中率 {Math.round(usage.hitRate * 100)}%</span>}
      ...
      <button type="button" onClick={() => { onSettings(); onClose(); }}><span /><Settings2 />运行设置</button>
      {conversation && <button type="button" onClick={() => { onRename(conversation); onClose(); }}><span /><Pencil />重命名</button>}
      {conversation && <button type="button" className="destructive" onClick={() => { onDelete(conversation); onClose(); }}><span /><Trash2 />删除会话</button>}
    </section>
  </div>;
}
...
export default function MobileWorkbench() {
  ...
  const [workerSwitchOpen, setWorkerSwitchOpen] = useState(false);
  ...
  const [detailFailures, setDetailFailures] = useState({});
  const [sessionActions, setSessionActions] = useState(null);
  const [usage, setUsage] = useState({ items: [], loading: false, error: "" });
  ...
  // Turns whose body has not arrived say so. Without this the phone renders a
  // half-loaded session as its first and last message and nothing else.
  // The board is read when it is opened and when the reader asks again. It is
  // the host's own account data, so there is nothing to poll for.
  const loadUsage = useCallback(async (refresh = false) => {
    if (!selectedWorkerID) return;
    setUsage((current) => ({ ...current, loading: true, error: "" }));
    try {
      const items = await MobileBinding.AccountUsage(selectedWorkerID, refresh);
      setUsage({ items: items || [], loading: false, error: "" });
    } catch (error) { setUsage({ items: [], loading: false, error: errorMessage(error) }); }
  }, [selectedWorkerID]);

  const markDetailFailure = useCallback((id, failed) => {
    setDetailFailures((current) => (Boolean(current[id]) === failed ? current : { ...current, [id]: failed }));
  }, []);

  const transcriptNotice = useCallback(
    (run) => mobileTranscriptNotice(run, loadedRunDetailsRef.current.get(run.id), Boolean(detailFailures[run.id])),
    [detailFailures],
  );
  ...
  // What a pull asks for: the machine, its projects and the run history, all
  // re-read at once instead of waiting for the next poll.
  ...
  useEffect(() => {
    ...
    for (const id of selectedConversationRunIDs.split("|")) {
      void MobileBinding.GetRun(id)
        .then((detail) => {
          if (stopped) return;
          loadedRunDetailsRef.current.set(id, mobileRunDetailRecord(detail, loadedRunDetailsRef.current.get(id)));
          markDetailFailure(id, false);
          setRuns((items) => mergeMobileRun(items, detail));
        })
        .catch((error) => {
          if (stopped) return;
          markDetailFailure(id, true);
          notify("error", `会话详情加载失败，将自动重试：${errorMessage(error)}`);
        });
    }
    return () => { stopped = true; };
  }, [selectedConversationRunIDs, markDetailFailure, notify]);
      ...
      try {
        ...
        for (const run of items || []) {
          ...
          try {
            ...
            if (stopped) return false;
            loadedRunDetailsRef.current.set(run.id, mobileRunDetailRecord(detail, loadedRunDetailsRef.current.get(run.id)));
            markDetailFailure(run.id, false);
            setRuns((current) => mergeMobileRun(current, detail));
          } catch {
            // Retry this body on the next poll; keep loading other turns.
            if (!stopped) markDetailFailure(run.id, true);
          }
        }
  ...
  }, [markDetailFailure, selectedConversationID, selectedWorkerID]);
  ...
  const startRun = async ({ resumeSessionId = "" } = {}) => {
    ...
    if (!text || startingRunRef.current) return false;
    setBusy("run");
    setPendingPrompt({ conversationID: selectedConversationID, workspaceID, workerID: selectedWorkerID, text, at: new Date().toISOString() });
    setPrompt("");
    try {
      const request = MobileBinding.StartRun({
      ...
      startingRunRef.current = request;
    ...
    finally { startingRunRef.current = null; setBusy(""); }
  };
  ...
  const queueFollowUp = async (runID) => {
    const text = prompt.trim();
    const target = runID ? Promise.resolve({ id: runID }) : startingRunRef.current;
    if (!text || !target) return false;
    const local = { id: `queuing-${++queueSequenceRef.current}`, text, pending: true, runID,
      conversationID: selectedConversationID, workspaceID, workerID: selectedWorkerID };
    // Attach immediately so a failed start is handled even while another queue
    // request is still in flight.
    const resolvedTarget = target.then((run) => {
      setQueuingPrompts((items) => items.map((item) => item.id === local.id ? { ...item, runID: run.id, conversationID: run.conversationId || run.id } : item));
      return run;
    }).catch(() => null);
    setQueuingPrompts((items) => [...items, local]);
    setPrompt("");
    const request = enqueueMobileFollowUp(queueTailRef.current, resolvedTarget, text,
      (id, message) => MobileBinding.QueueFollowUp(id, message));
    queueTailRef.current = request;
    try {
      const { runID: targetID, queued } = await request;
      setRuns((items) => items.map((item) => (item.id === targetID ? { ...item, queued } : item)));
      return true;
    } catch (error) {
  ...
  const renameConversation = async (conversation, title) => {
    setBusy("rename");
    try {
      await MobileBinding.RenameConversation(conversation.id, title);
      setRuns((items) => items.map((run) => (mobileConversationID(run) === conversation.id ? { ...run, title } : run)));
      setRenameTarget(null);
    } catch (error) { notify("error", errorMessage(error)); }
    finally { setBusy(""); }
  };

  // Deleting the open conversation leaves the transcript with nothing to show,
  // so the list it came from is where this lands.
  const performDeleteConversation = async (conversation) => {
    setBusy("delete");
    try {
      await MobileBinding.DeleteConversation(conversation.id);
      setRuns((items) => items.filter((run) => mobileConversationID(run) !== conversation.id));
      if (selectedConversationID === conversation.id) { setSelectedConversationID(""); setView("sessions"); }
      notify("success", "已删除会话");
    } catch (error) { notify("error", errorMessage(error)); }
    finally { setBusy(""); }
  };

  const deleteConversation = (conversation) => setConfirmRequest({
    title: `删除「${conversation.title}」`,
    body: "会话和它的全部运行记录都会从这台电脑上删除，无法撤销。",
    confirmLabel: "删除",
    destructive: true,
  ...
  // Switching machines changes what you are looking at, not what you are
  // doing: it stays on the project list instead of opening a new task.
  ...
  const headerIdentity = {
    ...
    usage: { title: "用量", meta: workerMeta },
  }[view] || { title: "" };
  const listRef = useRef(null);
  const listRefreshing = useMobilePullToRefresh(listRef, refreshLists, view === "projects" || view === "sessions");
  const goBack = ["conversation", "sessions", "workspaces", "usage"].includes(view) ? navigateBack : null;
  ...
  return <div className="mobile-app-shell" ref={shellRef}>
    ...
    <WorkerSwitchSheet open={workerSwitchOpen} workers={workers} selectedWorkerID={selectedWorkerID} healthByID={healthByID} onSelect={selectWorker} onPair={() => setPairTarget(null)} onClose={() => setWorkerSwitchOpen(false)} />
